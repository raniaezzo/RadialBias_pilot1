function [uEst,varEst] = FitCumNormYN_cg(inputs,nYes,nNo)
% same as FitCumNormYN, except Subproblem Algorithm is set to conjugate
% gradient -- this fixes rare errors in the fit

% [uEst,varEst] = FitCumNormYN_cg(inputs,nYes,nNo)
%
...
if (exist('fmincon') == 2)
    ...
    options = optimset(options,'SubproblemAlgorithm','cg'); % added this
...
elseif (exist('constr') == 2)
	...
	error('FitCumNormYN_cg requires the optional Matlab Optimization Toolbox from Mathworks');
end
